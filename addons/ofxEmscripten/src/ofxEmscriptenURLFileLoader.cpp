...
void ofxEmscriptenURLFileLoader::onload_cb(unsigned wat, void* request, void* data, unsigned int size){
...
void ofxEmscriptenURLFileLoader::onerror_cb(unsigned wat, void* request, int status, const char* message){
	ofHttpResponse response;
...
void ofxEmscriptenURLFileLoader::onload_file_cb(unsigned wat, void* request, const char* file){
	ofHttpResponse response;
	response.status = 200;
	response.request = *(ofHttpRequest*)request;
	ofNotifyEvent(ofURLResponseEvent(),response);
...
void ofxEmscriptenURLFileLoader::onerror_file_cb(unsigned wat, void* request, int status){
	ofHttpResponse response;
	response.status = status;
	response.request = *(ofHttpRequest*)request;
	ofNotifyEvent(ofURLResponseEvent(),response);
